...
interface FFALicenseResponse {
  ...
  hasPPS: boolean;
  ppsNumber?: string; // Numéro PPS pour non-licenciés
...
export async function verifyFFALicense(
  ...
  try {
    ...
    const result: FFALicenseResponse = {
      ...
      ppsNumber: data.numero_pps || undefined,
...
export async function updateAthletFFAInfo(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      pps_number: ffaData.ppsNumber,
      pps_expiration_date: ffaData.ppsExpirationDate,
      pps_valid_until: ffaData.ppsExpirationDate,
...
export async function verifyPPS(
  ...
  try {
    ...
    console.error('Error verifying PPS:', error);
    return { valid: false, reason: 'Erreur lors de la vérification du PPS' };
  }
}
...
/**
 * Formate un numéro PPS pour affichage
 */
export function formatPPS(pps: string): string {
  const clean = pps.replace(/[\s-]/g, '');
  if (clean.length === 9) {
