...
export default function ExternalEventResults() {
  const { slug } = useParams<{ slug: string }>();
  const [race, setRace] = useState<ExternalEvent | null>(null);
    ...
    try {
      ...
      while (hasMore) {
        const { data: resultsData, error: resultsError } = await supabase
          ...
          .select('*')
          .eq('external_event_id', raceData.id)
          .eq('status', 'finished')
  ...
  const calculateStats = () => {
    ...
    console.log('First result finish_time:', filteredResults[0]?.finish_time_display);

    const validResults = filteredResults.filter(r => r.finish_time_display && timeToSeconds(r.finish_time_display) > 0);
    ...
    const sortedTimes = [...validResults].sort((a, b) => {
      const timeA = timeToSeconds(a.finish_time_display);
      const timeB = timeToSeconds(b.finish_time_display);
      return timeA - timeB;
    });

    console.log('Sorted times first:', sortedTimes[0]?.finish_time_display);
    console.log('Sorted times last:', sortedTimes[sortedTimes.length - 1]?.finish_time_display);

    const medianIndex = Math.floor(sortedTimes.length / 2);
    const medianTime = sortedTimes.length % 2 === 0
      ? sortedTimes[medianIndex - 1]?.finish_time_display
      : sortedTimes[medianIndex]?.finish_time_display;
    ...
    const raceStats: RaceStats = {
      ...
      by_category: {},
      avg_time: '00:00:00',
      fastest_time: sortedTimes[0]?.finish_time_display || '00:00:00',
      median_time: medianTime || '00:00:00',
      slowest_time: sortedTimes[sortedTimes.length - 1]?.finish_time_display || '00:00:00',
      avg_age: 0,
      avg_age_male: 0,
      avg_age_female: 0
    };
    ...
    filteredResults.forEach((result) => {
      ...
      if (result.birth_date) {
        const birthYear = new Date(result.birth_date).getFullYear();
        age = new Date().getFullYear() - birthYear;
      } else if (result.birth_year) {
        age = new Date().getFullYear() - result.birth_year;
    ...
    console.log('Age stats - Total:', countAge, 'avg:', raceStats.avg_age);
    console.log('Age stats - Male:', countMale, 'avg:', raceStats.avg_age_male);
    console.log('Age stats - Female:', countFemale, 'avg:', raceStats.avg_age_female);
    console.log('Sample birth_year:', filteredResults.slice(0, 3).map(r => r.birth_year));
  ...
  const filterResults = () => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        r => r.last_name + " " + r.first_name.toLowerCase().includes(term) ||
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const resultsForPDF = filteredResults.map(r => ({
        ...r,
        athlete_name: `${r.last_name || ''} ${r.first_name || ''}`.trim(),
        finish_time: r.finish_time_display || '-'
      }));

      await generateResultsPDF(resultsForPDF, {
        event_name: event.name || 'Événement',
        race_name: race.name || 'Course',
        distance: (race.distance_km || 0) * 1000,
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-pink-50/30 overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-8 mb-8 border border-gray-100">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 border-t pt-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 border-t pt-6">
                ...
                return (
                  <div key={result.id} className={`${heights[idx]} relative overflow-hidden rounded-xl md:rounded-2xl shadow-xl transition-all duration-300 hover:scale-105`}>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-2 md:p-4">
                      ...
                      <p className="font-bold text-sm md:text-2xl text-white text-center drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] leading-tight px-1 md:px-2">{result.last_name + " " + result.first_name}</p>
                      <p className="text-xl md:text-4xl font-black text-white mt-2 md:mt-3 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]" style={{ fontFamily: 'system-ui, -apple-system, sans-serif', letterSpacing: '0.05em' }}>
                        {formatTime(result.finish_time_display)}
                      </p>
                      {race && race.distance_km > 0 && (() => {
                        const timeInSeconds = timeToSeconds(result.finish_time_display);
                        if (timeInSeconds > 0) {
                          const speedKmh = (race.distance_km / (timeInSeconds / 3600)).toFixed(2);
                          return (
        ...
        <div
          className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border border-gray-100 select-none"
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {code && (
                          <img
                            ...
                            alt={nat}
                            title={nat}
                            className="inline-block rounded shadow-sm flex-shrink-0"
                          />
                        )}
                      </div>
                      {result.club && (
                        <div className="text-xs text-gray-600 mb-1 truncate">{result.club}</div>
                      )}
                    ...
                    <div className="flex-shrink-0 text-right">
                      ...
                      {result.average_speed_kmh && (
                        <div className="text-xs text-blue-600 font-semibold">
                          {result.average_speed_kmh.toFixed(2)} km/h
                        </div>
                      )}
                      ...
                      {result.average_speed_kmh ? (
                        <div className="flex flex-col items-center">
                          <span className="text-xs font-semibold text-blue-600">
                            {result.average_speed_kmh.toFixed(1)}
                          </span>
                          <span className="text-[10px] text-gray-500">
                            {(() => {
                              const paceMinPerKm = 60 / parseFloat(result.average_speed_kmh.toString());
