/*
  # Fonction d'inscription atomique avec gestion des quotas

  1. Objectif
    - Gérer les inscriptions de manière atomique pour éviter les surréservations
    - Vérifier les quotas en temps réel avec verrouillage (FOR UPDATE)
    - Garantir qu'une course ne peut pas dépasser max_participants

  2. Fonctionnement
    - Lock la course pour éviter les conditions de concurrence
    - Vérifie le nombre d'inscrits confirmés vs max_participants
    - Insère l'athlète et l'inscription en transaction atomique
    - Retourne l'ID de l'inscription ou une erreur si course complète

  3. Sécurité
    - Fonction SECURITY DEFINER pour bypass RLS temporairement
    - Validation stricte des quotas
    - Gestion des erreurs explicite

  4. Performance
    - Utilise FOR UPDATE SKIP LOCKED pour éviter les deadlocks
    - Index déjà existants sur race_id et status
    - Transaction courte pour minimiser les locks
*/

-- Fonction pour créer une inscription avec vérification atomique des quotas
...
-- Créer un index pour optimiser le comptage des inscrits
CREATE INDEX IF NOT EXISTS idx_entries_race_status 
ON entries(race_id, status) 
WHERE status IN ('confirmed', 'pending');

-- Commenter la fonction
COMMENT ON FUNCTION register_athlete_with_quota_check IS 
'Inscrit un athlète à une course en vérifiant atomiquement les quotas. Utilise FOR UPDATE pour éviter les surréservations.';
