/*
  # Fix athlete duplicate constraint issue

  1. Changes
    - Modify register_athlete_with_quota_check to handle duplicate athletes
    - Use INSERT ... ON CONFLICT to update existing athlete or create new one
    - This prevents the "duplicate key value violates unique constraint idx_athletes_identity" error

  2. Security
    - No changes to RLS policies
*/

-- Drop and recreate the function with ON CONFLICT handling
...
BEGIN
  ...
  -- 4. INSÉRER ou METTRE À JOUR l'athlète avec ON CONFLICT
