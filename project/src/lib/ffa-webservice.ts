...
interface FFAWebserviceResponse {
  ...
  ppsflg: 'O' | 'N';             // PPS requis (O=Oui, N=Non)
...
export async function verifyFFALicenseForRace(
  ...
  try {
    ...
    const requiresPPS = ffaResponse.ppsflg === 'O';
    const requiresCertificate = requiresPPS || ffaResponse.relflg === 'N';
