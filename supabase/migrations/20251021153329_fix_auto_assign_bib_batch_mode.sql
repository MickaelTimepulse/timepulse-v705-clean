/*
  # Correction de l'attribution automatique des dossards

  1. Problème
    - La fonction auto_assign_bib_number() ne s'exécute pas en mode BATCH
    - Les inscriptions en mode BATCH ne reçoivent pas de dossards automatiquement
    
  2. Solution
    - Modifier la fonction pour ignorer aussi le mode BATCH
    - Seul le mode MANUAL empêche l'attribution automatique
    
  3. Note
    - En mode BATCH, les dossards sont attribués au fur et à mesure des inscriptions
    - L'organisateur peut toujours modifier les dossards manuellement après
*/
...
BEGIN
  ...
  IF bib_config IS NULL OR bib_config.mode = 'MANUAL' OR bib_config.mode = 'BATCH' THEN
    RETURN NEW;
  END IF;
