/*
  # Fix calculate_timepulse_index Distance Column
  
  The function references ra.distance_km but the column is named ra.distance
  This migration fixes the function to use the correct column name.
  
  Changes:
  - Replace distance_km with distance in calculate_timepulse_index function
*/
...
BEGIN
  ...
  WHERE r.athlete_id = p_athlete_id
    ...
    AND ra.distance BETWEEN 9.5 AND 10.5;  -- FIXED: distance_km -> distance
  ...
  -- 4. POLYVALENCE (10%)
  -- ============================================
  -- Simplified: count distinct sport_type from races
  ...
  WHERE athlete_id = p_athlete_id
    AND status = 'finished'
    AND (rank_scratch <= 3 OR overall_rank <= 3);
...
COMMENT ON FUNCTION calculate_timepulse_index(uuid) IS 
  'Calculate Timepulse index for an athlete based on performance, progression, regularity, versatility and podiums';
