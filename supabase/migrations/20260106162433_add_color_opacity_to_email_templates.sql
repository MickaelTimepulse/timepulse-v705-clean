/*
  # Ajouter l'opacité de la couleur de fond aux templates d'emails

  1. Modifications
    - Ajouter la colonne `color_opacity` à la table `email_templates`
    - Mettre à jour la fonction `admin_update_email_template` pour gérer le nouveau paramètre
    - Définir 50% comme valeur par défaut pour l'opacité de la couleur

  2. Sécurité
    - La fonction reste protégée par la vérification admin
*/

-- Ajouter la colonne color_opacity avec une valeur par défaut de 50
ALTER TABLE email_templates
ADD COLUMN IF NOT EXISTS color_opacity integer DEFAULT 50 CHECK (color_opacity >= 0 AND color_opacity <= 100);

-- Mettre à jour tous les templates existants qui n'ont pas encore de color_opacity
UPDATE email_templates
SET color_opacity = 50
WHERE color_opacity IS NULL;

-- Supprimer l'ancienne fonction admin_get_email_templates
DROP FUNCTION IF EXISTS admin_get_email_templates();

-- Recréer la fonction admin_get_email_templates avec color_opacity
...
BEGIN
  ...
  SELECT is_super_admin(auth.uid()) INTO v_is_admin;
...
-- Mettre à jour la fonction admin_update_email_template
