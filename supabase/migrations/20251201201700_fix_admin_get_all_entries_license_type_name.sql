/*
  # Corriger admin_get_all_entries pour afficher le nom du type de licence

  1. Modifications
    - Ajouter une jointure avec license_types
    - Retourner le nom lisible au lieu de l'UUID
    - Retourner aussi le code de la licence
*/

-- Supprimer l'ancienne fonction
DROP FUNCTION IF EXISTS admin_get_all_entries();

-- Recréer avec jointure sur license_types
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(lt.name, a.license_type)::text as license_type,
    lt.code::text as license_type_code,
    e.pps_number::text,
    e.pps_expiry_date
  FROM entries e
