/*
  # Amélioration de l'import en batch avec calcul automatique des catégories FFA

  1. Modifications
    - Calcul automatique des catégories FFA pendant l'import
    - Basé sur la date de naissance et la date de l'événement
    - Pas besoin de recalcul après l'import
*/
...
DECLARE
  ...
  v_year_end DATE;
BEGIN
  -- Calculer l'âge au 31 décembre de l'année de la course
  v_year_end := DATE(EXTRACT(YEAR FROM p_event_date) || '-12-31');
  v_age := EXTRACT(YEAR FROM AGE(v_year_end, p_birthdate));

  -- Catégories FFA
  RETURN CASE
    WHEN v_age <= 9 THEN 'EA'    -- École d'Athlétisme
    WHEN v_age <= 11 THEN 'PO'   -- Poussins
    WHEN v_age <= 13 THEN 'BE'   -- Benjamins
    WHEN v_age <= 15 THEN 'MI'   -- Minimes
    WHEN v_age <= 17 THEN 'CA'   -- Cadets
    WHEN v_age <= 19 THEN 'JU'   -- Juniors
    WHEN v_age <= 22 THEN 'ES'   -- Espoirs
    WHEN v_age <= 39 THEN 'SE'   -- Seniors
    WHEN v_age <= 49 THEN 'M0'   -- Masters 0
    WHEN v_age <= 54 THEN 'M1'   -- Masters 1
    WHEN v_age <= 59 THEN 'M2'   -- Masters 2
    WHEN v_age <= 64 THEN 'M3'   -- Masters 3
    WHEN v_age <= 69 THEN 'M4'   -- Masters 4
    WHEN v_age <= 74 THEN 'M5'   -- Masters 5
    WHEN v_age <= 79 THEN 'M6'   -- Masters 6
    WHEN v_age <= 84 THEN 'M7'   -- Masters 7
    WHEN v_age <= 89 THEN 'M8'   -- Masters 8
    WHEN v_age <= 94 THEN 'M9'   -- Masters 9
    ELSE 'M10'                    -- Masters 10+
  END;
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Mettre à jour la fonction d'import pour utiliser le calcul FFA
    ...
    BEGIN
      ...
      IF v_athlete_id IS NULL THEN
        INSERT INTO athletes (
          ...
          phone, city, postal_code, license_number, license_club, license_type,
          federation, league, club_number, pps_number, pps_valid_until
        ) VALUES (
          v_email, v_first_name, v_last_name,
          COALESCE(v_participant->>'gender', 'M'),
          v_birthdate,
          COALESCE(v_participant->>'nationality', 'FRA'),
          v_participant->>'phone', v_participant->>'city', v_participant->>'postal_code',
          v_participant->>'license_number', v_participant->>'license_club', v_participant->>'license_type',
          v_participant->>'federation', v_participant->>'league', v_participant->>'club_number',
          v_participant->>'pps_number', (v_participant->>'pps_valid_until')::DATE
        )
